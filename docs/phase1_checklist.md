...
PC (노트북, Tera Term)
   ...
   1. UART (PC ↔ MCU) — 외부 관찰 채널. tx_count, rx_count, 상태값을 Tera Term으로 본다. 이건 그냥 "디버그 창문"이지 검증 대상이 아니다.

    2. CAN (MCU 내부) — 진짜 검증 대상. bxCAN이 자기가 만든 프레임을 자기가 받을 수 있는가? 이게 Phase 1의 유일한 질문이야.
---
