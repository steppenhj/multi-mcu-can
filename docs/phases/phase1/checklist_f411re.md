...
MCP2515 모듈은 핀 레이블이 보드에 인쇄되어 있어 바로 확인 가능하다.
F411RE NUCLEO는 CN7/CN10 모르포 커넥터에 핀 번호만 있으므로, 배선 전에 핀맵을 참고해 실물 위치를 먼저 파악한다.

> **"NUCLEO-F411RE pinout"** 또는 **"STM32F411RE morpho connector"** 검색 → ST 공식 핀맵 이미지 참고.

찾아야 할 핀: `PB13`, `PB14`, `PB15`, `PB6`, `5V`, `GND`

- [ ] 핀맵에서 PB13 / PB14 / PB15 / PB6 / 5V / GND 위치 확인 후 실물 보드에 표시

### 배선표

| 색 | F411RE 핀 | 커넥터 | MCP2515 모듈 핀 | 신호 |
|---|-----------|--------|----------------|------|
| 빨강 | **5V** | CN6 pin 5 | VCC | 전원 |
| 검정 | **GND** | CN6 pin 6 | GND | 기준 전위 |
| 노랑 | **PB13** | CN10 | SCK | SPI2 클럭 |
| 초록 | **PB14** | CN10 | SO (MISO) | MCP2515 → F411RE |
| 파랑 | **PB15** | CN10 | SI (MOSI) | F411RE → MCP2515 |
| 하양(주황) | **PB6** | CN10 | CS | 칩 선택 (Active Low) |
