...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Main program body  (F411RE Phase 2 - MCP2515 Normal mode)
  ******************************************************************************
  * @attention
  *
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/* mcp2515_reset() -> 0xC0 명령으로 MCP2515 소프트 리셋 (Config 모드 진입) */
...
static void mcp2515_send_frame(uint16_t id, uint8_t *data, uint8_t len) {
    mcp2515_write(MCP_TXB0SIDH, (id >> 3) & 0xFF);   /* ID 상위 8비트 */
    mcp2515_write(MCP_TXB0SIDL, (id << 5) & 0xE0);   /* ID 하위 3비트 → [7:5] */
...
int main(void)
{
  ...
  /* CANSTAT 읽기 - Normal 모드(0x00) 진입 확인 */
  ...
  while (1)
  {
      ...
      if (mcp2515_read(MCP_CANINTF) & 0x01) {    /* RX0IF */
          ...
          /* 0x100 (F446RE 상태)도 수신하지만 별도 카운트 생략 */
      }
    /* USER CODE END WHILE */
