...
Phase 2 배선 완료 상태입니다. 좌측이 F411RE + MCP2515(SPI CAN), 우측이 F446RE + SN65HVD230(3.3V 트랜시버)이고, 중앙 브레드보드가 CAN_H / CAN_L / GND 공통 허브입니다.

---

## 왜 따로 repo를 팠나

부모 프로젝트에서 F446RE로 마이그레이션하다가 하드웨어 사고가 났습니다. 서보가 스톨됐고, GND 점퍼 선에 불이 붙었고, L298N 드라이버가 서보와 함께 망가졌습니다. 코드 문제가 아니었습니다. 하위 레이어를 바꿔놓고도 전에 잘 돌던 하드웨어는 그대로 멀쩡할 것이라고 가정한 게 문제였습니다.

그때 빠뜨렸던 규율 하나를 중심으로 이 repo를 다시 짰습니다. 다음 레이어를 올리기 전에 지금 레이어를 먼저 검증합니다.

---

## 운영 원칙

모든 Phase에 공통으로 지키는 규칙 세 가지입니다.

1. 코드를 의심하기 전에 전원과 GND부터 봅니다. 멀티미터로 확인하는 건 30초지만, 보드가 타면 그날치 작업이 날아갑니다.
2. 기대한 동작 없이 이상한 소리나 열이 나면 바로 전원을 끊습니다. 원인을 찾기 전엔 다시 넣지 않습니다. 스톨된 모터는 소리가 나기 전까지 조용합니다.
3. 한 번에 레이어 하나만 추가합니다. 버스가 새 것이면 펌웨어는 검증된 것을 쓰고, 펌웨어가 새 것이면 버스는 이미 검증돼 있어야 합니다.

---

## 아키텍처

CAN 2.0 버스 하나에 노드 둘, 500 kbps, 양쪽 물리적 끝단에 120Ω 종단 저항을 둡니다.
...
노드 역할은 아직 액추에이터가 없어서 자리만 잡아뒀습니다.
...
각 Phase는 독립적으로 빌드되고 회귀 테스트가 되는 산출물을 만듭니다. 뒤 Phase가 깨져도 앞 Phase는 돌아가야 합니다.
...
Phase 0를 따로 떼어 둔 것은, 위 사고가 아무도 안 보던 레이어를 타고 번졌기 때문입니다. 모든 보드가 깨끗하게 전원이 들어오고, GND가 연속이고, 버스에 붙이기 전에 UART로 "alive"가 찍히는 것을 확인하기 전엔 Phase 1을 시작하지 않습니다.
...
원래는 RPi5를 MCP2515 + TJA1050 모듈로 SPI를 통해 버스에 붙이려고 했는데, 두 가지가 걸렸습니다.

1. TJA1050 트랜시버가 5V 전용이고, 모듈 구조상 MCP2515 컨트롤러도 5V로 공급해야 합니다.
2. 그러면 MCP2515의 SPI 출력이 5V 로직이 되는데, RPi5 GPIO는 3.3V 전용이라 직결하면 손상될 수 있습니다. (이전에 이것으로 RPi5 8GB 한 대를 날렸습니다 — `docs/lesson_learned.md` 사고 1.)

해결책은 RPi 전용 CAN HAT(~25,000원, 제일 안전), 로직 레벨 컨버터, MCP2515 모듈 SMD 리워크 정도가 있는데, Phase 2가 끝나고 다시 보기로 했습니다. 운영 원칙 #3에 어긋나는 시점에서 무리하게 결정하지 않습니다.

---

## CAN ID 할당

진단 ID는 자동차 표준(UDS / ISO-15765) 관례를 따랐습니다.
...
CAN은 낮은 ID가 높은 우선순위로 중재됩니다. 하트비트를 제일 낮은 ID에 둬서, 버스가 붐벼도 노드 생존 신호가 밀리지 않게 했습니다.

전체 메시지 사전은 [`docs/specs/can_protocol.md`](docs/specs/can_protocol.md)에서 확인할 수 있습니다.
...
F446RE에 SN65HVD230을 쓴 이유: F446RE는 3.3V 로직 MCU입니다. 5V 전용 트랜시버(MCP2551, TJA1050)를 직결하면 TXD 입력의 VIH 임계값(~3.5V)을 3.3V 출력이 넘지 못해 간헐적으로 송신이 실패합니다. SN65HVD230은 3.3V 네이티브라 이 문제가 없습니다. Phase 2에서 F446RE 측에 새로 추가되는 변수를 물리 레이어 하나로 묶기 위한 선택이기도 합니다.

F411RE에 MCP2515를 쓴 이유: F411RE에는 bxCAN이 없습니다. MCP2515가 SPI로 제어하는 외부 CAN 컨트롤러라, F411RE가 CAN에 참여할 유일한 경로입니다. 모듈에 내장된 TJA1050(5V)은 CAN 버스 쪽에만 붙고, SPI MISO 선(5V → F411RE)은 PB14의 5V 내성으로 받습니다.

### F411RE(3.3V) ↔ MCP2515(5V) 전압 호환성

MCP2515 모듈은 5V로 공급하지만 F411RE(3.3V)와 레벨 컨버터 없이 직결됩니다. 방향별로 이유가 다릅니다.
...
부모 프로젝트 사고가 배터리 배선과 얽혀 있어서, 이 repo는 배터리 없이 USB 전원만 씁니다.

전체 BOM 및 배선은 [`docs/specs/hardware.md`](docs/specs/hardware.md)에서 확인할 수 있습니다.
...
#   - LED 1Hz 점멸
#   - UART로 "[F446RE] alive, t=12345ms" 0.5초마다 출력 (F411RE도 동일 패턴)
...
**결과 — 양방향 500 kbps 통신 검증 완료.** 두 노드가 물리 CAN 버스를 통해 서로의 하트비트(F446RE `0x010` ↔ F411RE `0x011`)를 수신하며, 양쪽 `peer_rx`가 `self_tx`와 동기 증가합니다. F446RE `err=0x0`, F411RE `eflg=0x00` (에러 플래그 없음)을 유지합니다.
...
> 영상이 재생되지 않으면 [`docs/assets/captures/Phase2_CAN.mp4`](docs/assets/captures/Phase2_CAN.mp4)에서 확인할 수 있습니다.

---

## 로드맵 (Phase 2 이후)

2노드 버스가 충분히 안정된 다음 생각하는 확장입니다.
